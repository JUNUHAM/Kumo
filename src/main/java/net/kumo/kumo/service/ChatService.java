...
public class ChatService {
         ...
         */
        public ChatRoomEntity createOrGetChatRoom(Long seekerId, Long recruiterId, Long targetPostId,
                        String targetSource) {
        ...
        public List<ChatRoomListDTO> getChatRoomsForUser(Long userId) {
                ...
                return rooms.stream()
                                // 1. 방(Room)과 그 방의 마지막 메시지(LastMessage)를 짝꿍으로 묶어줍니다.
                                .map(room -> {
                                        ChatMessageEntity lastMsg = chatMessageRepository
                                                        .findFirstByRoomOrderByCreatedAtDesc(room);
                                        return new java.util.AbstractMap.SimpleEntry<>(room, lastMsg);
                                })
                                // 2. ★ 대망의 정렬 로직 (최신 메시지 시간이 가장 큰 놈이 위로 가게 내림차순 정렬)
                                .sorted((entry1, entry2) -> {
                                        java.time.LocalDateTime time1 = entry1.getValue() != null
                                                        ? entry1.getValue().getCreatedAt()
                                                        : java.time.LocalDateTime.MIN;
                                        java.time.LocalDateTime time2 = entry2.getValue() != null
                                                        ? entry2.getValue().getCreatedAt()
                                                        : java.time.LocalDateTime.MIN;
                                        return time2.compareTo(time1); // time2가 앞으로 오게 (내림차순)
                                })
                                // 3. 정렬이 끝났으니 예쁘게 화면용 DTO로 포장해서 내보냅니다.
                                .map(entry -> {
                                        ChatRoomEntity room = entry.getKey();
                                        ChatMessageEntity lastMsg = entry.getValue();

                                        UserEntity opponent = room.getSeeker().getUserId().equals(userId)
                                                        ? room.getRecruiter()
                                                        : room.getSeeker();

                                        String profileImg = "/images/common/default_profile.png";
                                        if (opponent.getProfileImage() != null
                                                        && opponent.getProfileImage().getFileUrl() != null) {
        ...
        private ChatMessageDTO convertToDTO(ChatMessageEntity entity, String lang) {
                ...
                return ChatMessageDTO.builder()
                                ...
                                .senderNickname(entity.getSender() != null ? entity.getSender().getNickname()
                                                : messageSource.getMessage("chat.sender.unknown", null,
